...
import { getRecordedClasses, deleteRecordedClassById } from 'apiSdk/recorded-classes';
...
function RecordedClassListPage() {
  ...
  const handleView = (id: string) => {
    if (hasAccess('recorded_class', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/recorded-classes/view/${id}`);
    }
  };
  ...
  return (
    <AppLayout>
      <Box bg="white" p={4} rounded="md" shadow="md">
        {hasAccess('recorded_class', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <Flex justifyContent="space-between" mb={4}>
            <Text as="h1" fontSize="2xl" fontWeight="bold">
              Recorded Class
            </Text>
            <NextLink href={`/recorded-classes/create`} passHref legacyBehavior>
                  ...
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    <Td>
                      {hasAccess('recorded_class', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                        <NextLink href={`/recorded-classes/edit/${record.id}`} passHref legacyBehavior>
                          <Button
